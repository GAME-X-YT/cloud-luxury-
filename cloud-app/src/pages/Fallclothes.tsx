...
import { Loader2, Leaf, ThermometerSnowflake, Wind, ShoppingBag, Sparkles, Check } from "lucide-react";
...
const FallCard = ({ item, onAdd }: { item: FallItem; onAdd: () => void }) => {
  return (
    <motion.div 
      ...
      className="group relative"
    >
      <div className="relative aspect-4/5 overflow-hidden rounded-[2.5rem] bg-[#1a1512] border border-white/5 shadow-2xl transition-all duration-700 group-hover:border-orange-500/30 group-hover:shadow-orange-900/20">
    ...
    <div className="min-h-screen bg-[#0d0a09] text-white selection:bg-orange-500/30 overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none z-0">
        {[...Array(8)].map((_, i) => (
          ...
          >
            <Leaf size={20 + i * 10} strokeWidth={1} />
          </motion.div>
        ))}
      </div>
        ...
        <header className="mb-32 text-center md:text-left flex flex-col md:flex-row justify-between items-end gap-8">
          <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 1 }}>
            <div className="flex items-center gap-3 mb-6 justify-center md:justify-start">
              ...
              <span className="text-orange-500/80 text-[10px] font-bold tracking-[1em] uppercase">Collection 2025</span>
            </div>
          ...
          >
            <div className="bg-white text-black p-4 rounded-3xl shadow-2xl flex items-center justify-between border border-white/20 backdrop-blur-xl">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-[10px] uppercase tracking-widest font-black text-neutral-400 leading-none mb-1">Added to Wardrobe</p>
      ...
      <div className="h-64 bg-linear-to-t from-orange-950/20 to-transparent mt-20" />
    </div>
  );
};
