...
// import { login, loginOTPRequest } from "../api/api"; // Import the new OTP request function
...
//     // State to track if the OTP has been successfully requested and sent
...
//     // 1. Handler to request the OTP code (Step 1)
//     const handleRequestOtp = async () => {
//         setError('');
//         if (!form.email) {
//             setError("Please enter your email to request the login code.");
//             return;
//         }
//         setLoading(true);

//         try {
//             // Hits the backend POST /login-otp-request
//             const response = await loginOTPRequest({ email: form.email }); 
//             setOtpSent(true);
//             alert(response.data.message); // Should say "Code sent..."
//         } catch (err) {
...
//     // 2. Handler for the final login submission (Step 2: Email, Password, OTP)
//     const handleSubmit = async (e: React.FormEvent) => {
//         e.preventDefault();
//         setError('');

//         if (!otpSent || !form.otp) {
//             setError("Please request and enter the OTP code.");
//             return;
//         }

//         setLoading(true);
//         try {
//             // Hits the secured backend POST /login endpoint that checks all three
...
//         } catch (err: any) {
//              const message = err.response?.data?.message || "Login failed. Check credentials and OTP.";
//              setError(message);
//         } finally {
//             setLoading(false);
//         }
...
//                         disabled={otpSent || loading || !form.email}
//                         className="w-full bg-yellow-500 text-white font-semibold py-3 rounded-lg hover:bg-yellow-600 transition duration-300 shadow-md disabled:bg-gray-400 text-sm"
//                     >
//                         {loading && !otpSent ? <Spinner /> : null}
//                         {otpSent ? 'Code Sent (Check Email)' : 'Request Login Code'}
//                     </button>
//                 </div>
...
//                 {/* OTP Input (Visible only after OTP is requested) */}
//                 <input
//                     type="text"
//                     name="otp"
